...
class RotMagnifyingGlassView(ctx : Context) : View(ctx) {
    ...
    data class RMGNode(var i : Int, val state : State = State()) {

        private var next : RMGNode? = null
        private var prev : RMGNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawRMGNode(i, state.scale, paint)
        }
        ...
        fun getNext(dir : Int, cb : () -> Unit) : RMGNode {
            var curr : RMGNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class RotMagnifyingGlass(var i : Int) {

        private var curr : RMGNode = RMGNode(0)
    ...
    companion object {
        ...
        fun create(activity : Activity) : RotMagnifyingGlassView {
            val view : RotMagnifyingGlassView = RotMagnifyingGlassView(activity)
